...
def _load_and_process_coco_person_numpy_data(captions_file, image_dir):
  """Loads image metadata from a JSON file and processes the captions.
  ...
  Returns:
    A list of ImageMetadata.
  """
  caption_data = np.load(captions_file)
  # Extract the filenames.
  id_to_filename = [(x["id"], x["file_name"]) for x in caption_data]
  # Extract the captions. Each image_id is associated with multiple captions.
  id_to_captions = {}
  for annotation in caption_data:
    image_id = annotation["id"]
    caption = annotation["caption"][0]
    id_to_captions.setdefault(image_id, [])
    id_to_captions[image_id].append(caption)
  assert len(id_to_filename) == len(id_to_captions)
  assert set([x[0] for x in id_to_filename]) == set(id_to_captions.keys())
...
def main(unused_argv):
  ...
  train_dataset = _load_and_process_coco_person_numpy_data(FLAGS.train_captions_file,
  ...
  #   train_dataset = 100% of mscoco_train_dataset
