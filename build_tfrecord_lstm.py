...
tf.flags.DEFINE_string("captions_file",
                       "/home/zh/Transfer/flickr8kNonePerson/caption_no_person.txt",
                       "captions txt file.")

tf.flags.DEFINE_string("output_dir",
                       "/media/zh/E/transfer_no_person_TFRECORD_data_100000_test",
...
tf.flags.DEFINE_string("word_counts_output_file",
                       "/media/zh/E/transfer_no_person_TFRECORD_data_100000_test/word_counts.txt",
...
def _process_image_files(thread_index, ranges, name, images, vocab,
                         num_shards):
...
def _process_dataset(name, images, vocab, num_shards):
  ...
  images = [ImageMetadata(image.id, [caption])
...
def _load_and_process_metadata(captions_file):
    ...
    with open(captions_file, 'r') as f:
        for line in f:
            description = line.strip().split(' ')[1].replace('\n', '').replace('。','')
            descriptions.append(description)

    print("Loaded caption metadata for %d images from %s" %
          (len(descriptions), captions_file))
    ...
    for description in descriptions:
        captions = [_process_caption_jieba(description)]
        image_metadata.append(ImageMetadata(id, captions))
        id = id + 1
        num_captions += len(captions)
    print("Finished processing %d captions for %d images in %s" %
          (num_captions, len(descriptions), captions_file))
